...
if IS_WINDOWS and WINDOWS_AVAILABLE:
    ...
    class WindowsEffectHelper:
        ...
        def setBasicEffect(self, hWnd, hint):
            hWnd, margins = int(hWnd), MARGINS(-1, -1, -1, -1)
    ...
    class BaseWidget(QWidget):
        ...
        def mousePressEvent(self, e):
            if e.button() == Qt.LeftButton and self._pressToMove: self._move()
        ...
        def nativeEvent(self, e, message):
            ...
            if msg.message == win32con.WM_NCHITTEST and self._resizable:
                ...
                if max_f and not full_f:
                    ...
                    r.top += th; r.left += th; r.right -= th; r.bottom -= th
    ...
    class BaseWidget(QWidget):
        ...
        def _initVal(self):
            self._resizable, self._pressToMove = True, False
        ...
        def mousePressEvent(self, e):
            if e.button() == Qt.LeftButton:
                if self._resizing: self.window().windowHandle().startSystemResize(self._get_edge())
                elif self._pressToMove: self._move()
